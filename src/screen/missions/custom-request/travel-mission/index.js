...
const CustomAgentLocation = ({
  ...
  useEffect(() => {
    ...
    socket.on(`custom_agent_location_${id}`, (msg) => {
      console.log(msg, 'msg');
      setAgentLocation({
  ...
  return (
    <Block primary>
      <Header centerText={'Custom Mission'} bottomText={`MISN0${item.id}`} />
      <Block flex={1}>
        {location.latitude > 0 && (
          <MapView
            ref={mapRef}
...
export default CustomAgentLocation;
